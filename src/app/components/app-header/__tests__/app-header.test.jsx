...
describe('App header', () => {
    describe('Render', () => {
        it('Render app title if user is authenticated', () => {
            ...
            render(<AppHeader />)
            const appHeaderTitle = screen.getByText('Mandalas Moksha')
    ...
    describe('Behaviour', () => {
        ...
        it('Photography button redirects to paintings gallery when clicked', () => {
            ...
            const photographyGalleryLink = screen.getByTestId("custom-projects-redirect-link")
            expect(photographyGalleryLink).toBeInTheDocument();
            userEvent.click(photographyGalleryLink);
            expect(photographyGalleryLink.getAttribute("href")).toBe("/custom-projects");
        })
    })
})